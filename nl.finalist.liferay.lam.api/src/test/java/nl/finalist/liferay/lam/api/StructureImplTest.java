...
public class StructureImplTest {
	...
	public void shouldAddStructure_whenItDoesntExist() throws PortalException {
		...
		structure.createOrUpdateStructure(STRUCTURE_KEY, TEST_URL, mockBundle, nameMap, descriptionMap, null);
		
		verify(ddmStructureLocalService, times(1)).addStructure(eq(USER_ID), eq(GLOBAL_GROUP_ID), any(), eq(CLASSNAME_ID), eq(STRUCTURE_KEY), eq(nameMap), eq(descriptionMap), any(), any(), eq("json"), eq(0), any());
	}
	...
	public void shouldAddToGivenSite_whenSiteKeyIsSpecified() throws PortalException {
		...
		structure.createOrUpdateStructure(STRUCTURE_KEY, TEST_URL, mockBundle, nameMap, descriptionMap, SITE_KEY);
		
		verify(ddmStructureLocalService, times(1)).addStructure(eq(USER_ID), eq(GROUP_ID), any(), eq(CLASSNAME_ID), eq(STRUCTURE_KEY), eq(nameMap), eq(descriptionMap), any(), any(), eq("json"), eq(0), any());
	}
}
