...
public class TemplateImplTest {
    ...
    public void createTemplate() throws PortalException {
        ...
        when(bundle.getResource(any())).thenReturn(TemplateImplTest.class.getResource("/TestURL.html"));

        String templateKey = "myADTKey";
        /*
         * Mocked a company and used its webID Mocking company was already
         * implemented
         */
        Company mockCompany = mock(Company.class);
        template.createOrUpdateTemplate(new String[] {mockCompany.getWebId()}, templateKey, "/TestURL.html", bundle, null, null, null, null);
        verify(ddmTemplateLocalService).addTemplate(anyLong(), anyLong(), anyLong(), anyLong(), anyLong(), eq(templateKey), any(), any(),
                eq(DDMTemplateConstants.TEMPLATE_TYPE_DISPLAY), eq(DDMTemplateConstants.TEMPLATE_MODE_CREATE), anyString(), anyString(), eq(false),
    ...
    public void updateTemplate() throws PortalException {
         ...
         */
        Company mockCompany = mock(Company.class);
        template.createOrUpdateTemplate(new String[] {mockCompany.getWebId()}, existingTemplateKey, "/TestURL.html", bundle, null, null, null, null);
    ...
    public void createTemplateForSite() throws PortalException {
        ...
        // used mocked company's webID

        template.createOrUpdateTemplate(new String[] {mockCompany.getWebId()}, templateKey, "/TestURL.html", bundle, null, null, null, "test");
        verify(ddmTemplateLocalService).addTemplate(anyLong(), eq(123L), anyLong(), anyLong(), anyLong(), eq(templateKey), any(), any(),
                eq(DDMTemplateConstants.TEMPLATE_TYPE_DISPLAY), eq(DDMTemplateConstants.TEMPLATE_MODE_CREATE), anyString(), anyString(), eq(false),
                eq(false), anyString(), any(), any());
    }
}