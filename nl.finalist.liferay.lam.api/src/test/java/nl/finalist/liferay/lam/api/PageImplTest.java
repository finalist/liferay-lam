...
public class PageImplTest {
    ...
    PageModel pageModel = new PageModel(null, TEST_SITEKEY, false, nameMap, titleMap, descriptionMap, friendlyUrlMap, "", null, null, null, null);
    ...
    public void addPage_shouldAdd_whenPageDoesntExist() throws PortalException {
        ...
        when(mockLayoutService.addLayout(anyLong(), anyLong(), anyBoolean(), anyLong(), any(), any(), any(), any(), any(), anyString(), anyString(),
                anyBoolean(), any(), any())).thenReturn(layout);

        page.addPage(null, TEST_SITEKEY, pageModel);
    ...
    public void addPage_shouldUpdate_whenPageExists() throws PortalException {
        ...
        when(mockLayoutService.updateLayout(anyLong(), anyBoolean(), anyLong(), anyLong(), any(), any(), any(), any(), any(), anyString(),
                anyBoolean(), any(), anyBoolean(), any(), any())).thenReturn(layout);
        ...
        verify(mockLayoutService.updateLayout(GROUP_ID, false, 1L, LayoutConstants.DEFAULT_PARENT_LAYOUT_ID, LocaleMapConverter.convert(nameMap),
                titleMap, descriptionMap, null, null, "portlet", false, LocaleMapConverter.convert(friendlyUrlMap), false, null,
                new ServiceContext()));
    }
    ...
    public void addPage_shouldAddCustomFields_whenCustomFieldsHaveBeenSpecified() throws PortalException {
        ...
        verify(mockLayoutService).addLayout(anyLong(), anyLong(), anyBoolean(), anyLong(), any(), any(), any(), any(), any(), anyString(),
                anyString(), anyBoolean(), any(), any());
        verify(mockCustomFields).addCustomFieldValue(null, Layout.class.getName(), "test", 1L, "test");
    }
}
