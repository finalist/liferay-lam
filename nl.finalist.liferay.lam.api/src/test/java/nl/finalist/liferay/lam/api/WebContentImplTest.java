...
public class WebContentImplTest {
	...
	@Mock
	JournalArticleLocalService journalArticleLocalServie;
	@InjectMocks
	...
	public void testAddNewArticle() throws Exception {
		...
		descriptionMap.put(Locale.US, " desscription for US");
		...
		verify(journalArticleLocalServie).addArticle(anyLong(), anyLong(), anyLong(), anyMap(), anyMap(), anyString(),
	...
	public void testUpdateArticle() throws PortalException {
		...
		when(journalArticleLocalServie.fetchArticleByUrlTitle(10L,urlTitle)).thenReturn(journalArticle);
		webContentImpl.updateWebContent(newTitleMap, newDescriptionMap, content, urlTitle);
		ArgumentCaptor<ServiceContext> argument = ArgumentCaptor.forClass(ServiceContext.class);
		verify(journalArticleLocalServie).fetchArticleByUrlTitle(10L, urlTitle);
		verify(journalArticleLocalServie).updateArticle(anyLong(), anyLong(), anyLong(), anyString(), anyDouble(),
	...
	public void testUpdateAddNewArticle() throws PortalException {
		...
		String urlTitle = "Testing webcontent";
		journalArticle.setArticleId("1");
		when(journalArticleLocalServie.fetchArticleByUrlTitle(10L,urlTitle)).thenReturn(null);
		webContentImpl.updateWebContent( newTitleMap, newDescriptionMap, content, urlTitle);
		ArgumentCaptor<ServiceContext> argument = ArgumentCaptor.forClass(ServiceContext.class);
		verify(journalArticleLocalServie, never()).updateArticle(anyLong(), anyLong(), anyLong(), anyString(), anyDouble(),
				anyMap(), anyMap(), anyString(), anyString(), argument.capture());
		verify(journalArticleLocalServie).addArticle(anyLong(), anyLong(), anyLong(), anyMap(), anyMap(), anyString(),
				anyString(), anyString(), argument.capture());
	}
	...
	public void testDeleteArticle() throws PortalException {
		...
		verify(journalArticleLocalServie).fetchArticleByUrlTitle(10L, urlTitle);
		verify(journalArticleLocalServie).deleteJournalArticle(anyLong());
	}
	...
	public void testNotDeleteArticle() throws PortalException {
		...
		webContentImpl.deleteWebContent(urlTitle);
		verify(journalArticleLocalServie).fetchArticleByUrlTitle(10L, urlTitle);
		verify(journalArticleLocalServie, never()).deleteJournalArticle(anyLong());
	}
